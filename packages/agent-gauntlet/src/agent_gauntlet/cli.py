...
def _handle_okf(args: argparse.Namespace, workspace: Path) -> int:
    ...
    from agent_gauntlet.features.okf.cli import execute_okf_cli

    return execute_okf_cli(args, workspace)


def _handle_supervisor(args: argparse.Namespace, workspace: Path) -> int:
    """Dispatches supervisor daemon start and status subcommands."""
    from agent_gauntlet.features.supervisor.cli import execute_supervisor_cli

    return execute_supervisor_cli(args, workspace)
...
def build_cli_parser() -> argparse.ArgumentParser:
    ...
    sup_p = subs.add_parser("supervisor", parents=[common])
    sup_sub = sup_p.add_subparsers(dest="supervisor_subcommand", required=True)
    sup_start = sup_sub.add_parser("start", parents=[common])
    sup_start.add_argument("--socket-path", default="")
    sup_start.add_argument("--daemon", action="store_true", help="Run daemon in background")
    sup_start.add_argument("-t", "--task-id", default="", help="Active task id to bind")
    sup_stat = sup_sub.add_parser("status", parents=[common])
    sup_stat.add_argument("--socket-path", default="")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "supervisor":
        return _handle_supervisor(args, workspace)
    return 2
